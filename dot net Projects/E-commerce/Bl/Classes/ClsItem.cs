...
{
    public class ClsItem : BusinessLayerInterFace<TbItem>
    {
        private readonly LapShopContext context;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public ClsItem(IHttpContextAccessor _httpContextAccessors, LapShopContext ctx)
        {
            context = ctx;
        ...
        public List<TbItem> GetAll()
        {
            try
            {
                List<TbItem> photos = context.TbItems.Where(a => a.CurrentState == Convert.ToInt32(EntityStateEnum.Exists)).ToList();
                return photos;
            }
            catch (Exception ex)
            {
        ...
        public TbItem GetById(int Id)
        {
            try
            {
                TbItem item = context.TbItems.Where(a => a.ItemId == Id && a.CurrentState == Convert.ToInt32(EntityStateEnum.Exists)).FirstOrDefault();
                return item;
            }
            catch (Exception ex)
            {
