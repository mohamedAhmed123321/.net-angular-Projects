...
{
    public class ClsOs : BusinessLayerInterFace<TbO>
    {
        private readonly LapShopContext context;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public ClsOs(IHttpContextAccessor _httpContextAccessors, LapShopContext ctx)
        {
            context = ctx;
        ...
        public List<TbO> GetAll()
        {
            try
            {
                List<TbO> photos = context.TbOs.Where(a => a.CurrentState == Convert.ToInt32(EntityStateEnum.Exists)).ToList();
                return photos;
            }
            catch (Exception ex)
            {
        ...
        public TbO GetById(int Id)
        {
            try
            {
                TbO item = context.TbOs.Where(a => a.OsId == Id && a.CurrentState == Convert.ToInt32(EntityStateEnum.Exists)).FirstOrDefault();
                return item;
            }
            catch (Exception ex)
            {
