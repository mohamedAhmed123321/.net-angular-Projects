...
{
    public class ClsPage : BusinessLayerInterFace<TbPage>
    {
        private readonly LapShopContext context;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public ClsPage(IHttpContextAccessor _httpContextAccessors, LapShopContext ctx)
        {
            context = ctx;
        ...
        public List<TbPage> GetAll()
        {
            try
            {
                List<TbPage> photos = context.TbPages.Where(a => a.CurrentState == Convert.ToInt32(EntityStateEnum.Exists)).ToList();
                return photos;
            }
            catch (Exception ex)
            {
        ...
        public TbPage GetById(int Id)
        {
            try
            {
                TbPage item = context.TbPages.Where(a => a.PageId == Id && a.CurrentState == Convert.ToInt32(EntityStateEnum.Exists)).FirstOrDefault();
                return item;
            }
            catch (Exception ex)
            {
